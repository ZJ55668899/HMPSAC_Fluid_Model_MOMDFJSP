...
add_data_object.add_header(['epoch', 'makespan', 'episode_reward_sum', 'critic_loss',
...
win = window_name
title = window_name
vis.line(X=[0], Y=[650], win=win, opts=dict(title=title, xlabel='epoch', ylable='completion_time', font=dict(family='Times New Roman')))
...
class DA3C(Base_Agent, Config):
    ...
    def __init__(self):
        ...
        self.worker_processes = min(8, max(1, self.num_processes - 5))  # 启用线程数
    ...
    def run(self):
        ...
        for ep_ix in range(self.episodes_to_run):
            ...
            with self.counter.get_lock():
                ...
                add_data_object.add_data([self.counter.value, self.environment_test.completion_time,
